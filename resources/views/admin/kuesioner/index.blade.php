                ...
                <div class="bg-white rounded-4 shadow-sm p-4">
                    ...
                    <form id="globalSettingsForm">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="target_jurusan_id" class="form-select bg-light border-0 rounded-3">
                                <option value="">Semua Jurusan</option>
                                @foreach($jurusanList as $jurusan)
                                    <option value="{{ $jurusan->id }}" 
                                        {{ $targetJurusan == $jurusan->nama_jurusan ? 'selected' : '' }}>
                                        {{ $jurusan->nama_jurusan }}
                                    </option>
                                @endforeach
                            </select>
                            <div class="form-text">Pilih jurusan target untuk kuesioner</div>
                        </div>
                        <div class="mb-3">
                            ...
                            <tbody>
                                @forelse($periode as $p)
                                    <tr>
                                        ...
                                        <td>{{ \Carbon\Carbon::parse($p->tanggal_mulai)->format('d/m/Y') }} -
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm" role="group">
                                                <button class="btn btn-outline-info rounded-pill me-1 edit-period"
                                                    ...
                                                    data-target_role="{{ $p->target_role }}"
...
<script>
    ...
    document.getElementById('globalSettingsForm').addEventListener('submit', function (e) {
        ...
        axios.post('{{ route("admin.kuesioner.settings") }}', data)
            .then(res => { 
                if (res.data.success) Swal.fire('Berhasil', res.data.message, 'success'); 
    ...
    document.querySelectorAll('.edit-period').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            // Cari jurusan berdasarkan nama jurusan yang tersimpan
