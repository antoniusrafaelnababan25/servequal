                ...
                <div class="bg-white rounded-4 shadow-sm p-4">
                    <nav aria-label="breadcrumb" class="mb-4">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">
                                <a href="{{ route('admin.pertanyaan.show', $pertanyaan->id) }}"
                    ...
                    <div class="d-flex align-items-center mb-4">
                        ...
                        <div>
                            <h5 class="fw-semibold mb-0">Edit Pertanyaan</h5>
                            <p class="text-muted small mb-0">Ubah data pertanyaan yang sudah ada</p>
                        </div>
                    </div>
                    ...
                    <form id="questionForm">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="tipe_penilaian" id="tipe_penilaian"
                                    ...
                                    @foreach($tipePenilaianList as $key => $label)
                                        <option value="{{ $key }}" {{ $pertanyaan->tipe_penilaian == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="dimensi" id="dimensi" class="form-select bg-light border-0 rounded-3"
                                    ...
                                    @foreach($dimensiList as $key => $label)
                                        <option value="{{ $key }}" {{ $pertanyaan->dimensi == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="target_role" id="target_role" class="form-select bg-light border-0 rounded-3"
                                    ...
                                    @foreach($targetRoleList as $key => $label)
                                        <option value="{{ $key }}" {{ $pertanyaan->target_role == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3" id="kategori_fasilitas_container"
                                style="{{ $pertanyaan->tipe_penilaian == 'penilaian_fasilitas' ? '' : 'display: none;' }}">
                                <label class="form-label fw-semibold">
                                ...
                                <select name="kategori_fasilitas" id="kategori_fasilitas"
                                    ...
                                    @foreach($kategoriFasilitasList as $key => $label)
                                        <option value="{{ $key }}" {{ $pertanyaan->kategori_fasilitas == $key ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                                    ...
                                    <div class="card-body">
                                        <div class="form-check form-switch">
                                            <input type="checkbox" class="form-check-input" name="is_active" id="is_active"
                                                value="1" {{ $pertanyaan->is_active ? 'checked' : '' }}>
                                            <label class="form-check-label fw-semibold ms-2" for="is_active">
                                                <i class="bi bi-check-circle-fill me-1 text-success"></i>Aktifkan Pertanyaan
                                            </label>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="d-flex gap-3 mt-4 pt-3 border-top">
                            <button type="submit" class="btn btn-purple rounded-pill px-5 py-2" id="submitBtn">
                                <i class="bi bi-save me-2"></i>Update Pertanyaan
                            </button>
                            <a href="{{ route('admin.pertanyaan.show', $pertanyaan->id) }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('questionForm').addEventListener('submit', function (e) {
                ...
                axios.post('{{ route("admin.pertanyaan.update", $pertanyaan->id) }}', new FormData(this), {
                    ...
                    .then(res => {
                        ...
                        if (res.data.success) {
                            Swal.fire('Berhasil!', res.data.message, 'success').then(() => {
                                window.location.href = '{{ route("admin.pertanyaan.show", $pertanyaan->id) }}';
                            });
                        } else {
                            Swal.fire('Gagal!', res.data.message, 'error');
                            submitBtn.disabled = false;
                            submitBtn.innerHTML = '<i class="bi bi-save me-2"></i>Update Pertanyaan';
                        }
                    })
                    .catch(err => {
                        Swal.close();
