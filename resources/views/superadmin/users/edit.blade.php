...
@section('content')
    <div class="container-fluid px-0">
        ...
        <div class="bg-white rounded-4 shadow-sm p-4">
            <form id="editUserForm" action="{{ route('super.users.update', $user->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                ...
                <div id="dosenFields" style="display: none;">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="prodi_id" id="prodi_dosen" class="form-select rounded-3">
                                <option value="">-- Pilih Jurusan Terlebih Dahulu --</option>
                                @if($user->prodi_id && $user->prodi)
                                    <option value="{{ $user->prodi_id }}" selected>{{ $user->prodi->nama_prodi }}</option>
                                @endif
                            </select>
                        </div>
                        <div class="col-md-6 mb-3">
                ...
                <div id="mahasiswaFields" style="display: none;">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label fw-semibold">Program Studi</label>
                            <select name="prodi_id" id="prodi_mahasiswa" class="form-select rounded-3">
                                <option value="">-- Pilih Jurusan Terlebih Dahulu --</option>
                                @if($user->prodi_id && $user->prodi)
                                    <option value="{{ $user->prodi_id }}" selected>{{ $user->prodi->nama_prodi }}</option>
                                @endif
...
@push('scripts')
    ...
    <script>
        (function () {
            ...
            const currentJurusanId = '{{ $user->jurusan }}';
            const currentProdiId = '{{ $user->prodi_id }}';
            ...
            function toggleFieldsByRole(role) {
                ...
                if (role === 'super_admin' || role === 'admin') {
                    ...
                    if (currentJurusanId && currentJurusanId !== '') {
                        loadProdi(currentJurusanId, prodiDosen, currentProdiId);
                    }
                } else if (role === 'mahasiswa') {
                    mahasiswaFields.style.display = 'block';
                    if (currentJurusanId && currentJurusanId !== '') {
                        loadProdi(currentJurusanId, prodiMahasiswa, currentProdiId);
                    }
                }
            }

            function loadProdi(jurusanId, targetSelect, selectedProdiId = null) {
                if (!jurusanId) {
                ...
                axios.get('/super-admin/users/prodi-by-jurusan/' + jurusanId)
                    .then(response => {
                        ...
                        response.data.forEach(prodi => {
                            const selected = (selectedProdiId && selectedProdiId == prodi.id) ? 'selected' : '';
                            targetSelect.innerHTML += `<option value="${prodi.id}" ${selected}>${prodi.nama_prodi} (${prodi.jenjang})</option>`;
                        });
            ...
            // Initialize
            toggleFieldsByRole(currentRole);
