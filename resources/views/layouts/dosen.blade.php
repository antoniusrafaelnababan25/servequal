...
<head>
    ...
    <title>@yield('title', 'SERVQUAL Dosen | POLMED')</title>
...
<body class="bg-light">
    ...
    <aside
        ...
        <div class="p-5 border-b border-white/10">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-purple-300 small mb-0">Dosen Panel</p>
                </div>
            </div>
        </div>
        ...
        <nav class="mt-4 px-3">
            <a href="{{ route('dosen.dashboard') }}"
                class="d-flex align-items-center gap-3 px-3 py-2 rounded-3 transition-all {{ request()->routeIs('dosen.dashboard') ? 'nav-active bg-white/10' : 'hover-bg-white/5' }}"
                style="text-decoration: none; color: inherit;">
                <i
                    class="bi bi-speedometer2 fs-5 {{ request()->routeIs('dosen.dashboard') ? 'text-purple-300' : 'text-gray-400' }}"></i>
                <span class="small fw-medium">Dashboard</span>
            </a>
            <a href="{{ route('dosen.kuesioner-fasilitas.create') }}"
                class="d-flex align-items-center gap-3 px-3 py-2 rounded-3 transition-all mt-1 {{ request()->routeIs('dosen.kuesioner-fasilitas.*') ? 'nav-active bg-white/10' : 'hover-bg-white/5' }}"
                style="text-decoration: none; color: inherit;">
                <i
                    class="bi bi-file-text fs-5 {{ request()->routeIs('dosen.kuesioner-fasilitas.*') ? 'text-purple-300' : 'text-gray-400' }}"></i>
                <span class="small fw-medium">Kuesioner Fasilitas</span>
            </a>
            <a href="{{ route('dosen.penilaian-mahasiswa.index') }}"
                class="d-flex align-items-center gap-3 px-3 py-2 rounded-3 transition-all mt-1 {{ request()->routeIs('dosen.penilaian-mahasiswa.*') ? 'nav-active bg-white/10' : 'hover-bg-white/5' }}"
                style="text-decoration: none; color: inherit;">
                <i
                    class="bi bi-star fs-5 {{ request()->routeIs('dosen.penilaian-mahasiswa.*') ? 'text-purple-300' : 'text-gray-400' }}"></i>
                <span class="small fw-medium">Penilaian Mahasiswa</span>
            </a>
        </nav>
                ...
                <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="notif-list" style="max-height: 400px; overflow-y: auto;">
                                    @forelse(($notifikasi ?? []) as $notif)
                                        <div class="px-3 py-2 border-bottom notif-item cursor-pointer"
                                            ...
                                            <p class="small text-gray-400 mt-1 mb-0">
                    ...
                    <div class="position-relative" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            <a href="{{ route('profile.edit') }}" class="dropdown-item px-3 py-2 small"><i
                                    class="bi bi-person me-2"></i>Profile</a>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="dropdown-item px-3 py-2 small text-danger"><i
    ...
    <div class="mobile-nav d-lg-none">
        <a href="{{ route('dosen.dashboard') }}"
            class="nav-item {{ request()->routeIs('dosen.dashboard') ? 'active' : '' }}">
            <i class="bi bi-speedometer2"></i><span>Dashboard</span>
        </a>
        <a href="{{ route('dosen.kuesioner-fasilitas.create') }}"
            class="nav-item {{ request()->routeIs('dosen.kuesioner-fasilitas.*') ? 'active' : '' }}">
            <i class="bi bi-file-text"></i><span>Kuesioner</span>
        </a>
        <a href="{{ route('dosen.penilaian-mahasiswa.index') }}"
            class="nav-item {{ request()->routeIs('dosen.penilaian-mahasiswa.*') ? 'active' : '' }}">
            <i class="bi bi-star"></i><span>Penilaian</span>
        </a>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
    ...
    <script>
        (function () {
            ...
            document.querySelectorAll('.notif-item').forEach(el => {
                el.addEventListener('click', function () {
                    ...
                    if (!id) return;
                    axios.post('/dosen/notifikasi/' + id + '/read')
        ...
        function markAllNotificationsRead() {
            axios.post('{{ route("dosen.notifikasi.read-all") }}')
                .then(() => location.reload())
                .catch(err => console.error(err));
        }
    </script>
